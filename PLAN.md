...
| Frontend | React (+ a chart lib, e.g. Recharts) | Reuses existing React experience; talks to the C++ server's API |
...
├── dashboard/             React admin dashboard (added in Phase 9)
├── Dockerfile
└── docker-compose.yml
```
...
## 6. Design Patterns (used deliberately, not for a checklist)
